...
void performSlicing(const MatrixXd &V, const MatrixXi &F, const CutNode *cuttree, vector<MatrixXd> &piecesV, vector<MatrixXi> &piecesF)
{
	...
	MatrixXd upperV;
	MatrixXi upperF;
	igl::copyleft::boolean::mesh_boolean(V,F,cubeV,cubeF,igl::copyleft::boolean::MESH_BOOLEAN_TYPE_INTERSECT,upperV,upperF);
	makeCube(V, cuttree->pt, -cuttree->n, 0.1, cubeV, cubeF);
	MatrixXd lowerV;
	MatrixXi lowerF;
	igl::copyleft::boolean::mesh_boolean(V,F,cubeV,cubeF,igl::copyleft::boolean::MESH_BOOLEAN_TYPE_INTERSECT,lowerV,lowerF);
